...
new_sample1<-mysample   # copies the dataset
piping<-new_sample1 %>% 
  arrange(airline) %>%                           # arranges as per airline order
  filter(incidents_85_99<25) %>%                 #filters incidents_85_89 column which are less than 25
  rename(seats = avail_seat_km_per_week) %>%      # renames the column name
  select(incidents_00_14, incidents_85_99) %>%    # selects only two columns 
  summary()%>%                                     # calculates the descriptive stats
